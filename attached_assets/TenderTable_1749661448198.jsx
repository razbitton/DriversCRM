...
export default function TenderTable({ tenders, onStop, onEdit, onDelete }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div>
        {tenders.map((tender) => {
          const statusStyle = statusConfig[tender.status] || statusConfig.available;
          ...
          return (
            <div key={tender.id} className="tender-table-row">
              ...
              <div>
                {serviceTypeLabels[tender.service_type] || tender.service_type}
              </div>
              <div className="text-gray-600">
                {format(new Date(tender.publication_time), "dd.MM.yyyy HH:mm")}
              </div>
              <div>
              ...
              <div className="text-sm">
                {tender.assigned_driver_name ? (
                  <>
                    <span className="font-medium">{tender.assigned_driver_name}</span>
                    {tender.assigned_driver_phone && (
                      <span className="text-gray-500"> | {tender.assigned_driver_phone}</span>
                    )}
                  </>
                ) : (
