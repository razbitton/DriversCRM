...
import DriverSelector from '../Components/custom-report/DriverSelector';
...
const CustomReportTable = ({ reports }) => {
...
export default function CustomReport() {
  ...
  useEffect(() => {
    const loadReports = async () => {
      try {
        const data = await DriverReportEntity.list("-created_date", 5);
        setReports(data);
      } catch (error) {
    ...
    // Simulate fetching all drivers - in a real app, this would be an API call
    const fetchAllDrivers = async () => {
      // Example: const drivers = await DriverEntity.listAll();
  ...
  const handleDriversChange = (drivers) => {
  ...
  const handleSearch = () => {
    ...
    // כאן תהיה הלוגיקה לחיפוש והסינון
  };
