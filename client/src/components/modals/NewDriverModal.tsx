...
import type { InsertDriver, Client } from '@shared/schema';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const driverData: InsertDriver = {
        ...
        license_number: formData.vehicle_license || null,
        license_expiry: null,
        vehicle_type: formData.vehicle_type || null,
        vehicle_license: formData.vehicle_license || null,
        vehicle_year: null,
        status: 'active',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg max-w-4xl max-h-[90vh] overflow-hidden w-full mx-4">
        ...
        <footer className="modal-footer">
          ...
          >
            {createDriverMutation.isPending ? "שומר..." : "שמור"}
          </button>
        </footer>
      </div>
    </div>
