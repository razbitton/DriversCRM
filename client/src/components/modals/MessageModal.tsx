...
export default function MessageModal({ setOpen }: {
  ...
  const [formData, setFormData] = useState({
    ...
    assigned_driver_id: 'all_drivers',
    title: '',
    content: '',
  ...
  const handleTimeSearch = (value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.assigned_driver_id !== 'all_drivers') {
        const selectedDriver = drivers.find(d => d.id.toString() === formData.assigned_driver_id);
        if (selectedDriver) {
      ...
      if (formData.channel_id !== 'all_channels') {
          const selectedChannel = channels.find(c => c.id.toString() === formData.channel_id);
          if (selectedChannel) {
      ...
      const submissionData = {
        ...formData,
        channel_assignment: channelName,
      ...
      console.log("Sending message:", submissionData);
      setOpen(false);
    } catch (error) {
